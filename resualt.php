<?php
 ...
 if(Input::exists('oldr')){
  $asse_table = Input::get('year');
   ...
   $priorityset =  $user->data()->priority;
 ...
 if($priorityset == 2){
   ...
   $user->findoder('stud_de',[['stud_level', '=', $_SESSION['staff_d']->staff_cl] ,'AND', ['stud_subcl', '=' ,$_SESSION['staff_d']->staff_mcl]]);  
    ...
    $priority = ($priorityset == 4) ? 'rector_not' : 'stud_note';
...
<script>
window.location.replace("/useresult.php?studid=<?php echo Input::get('user') ;?>"); 
</script>
<?php }else{
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                   ...
                                   foreach ($over_pos as $ky => $va) { 
                                
                                    //   $pass =  (count($userpass[$ky]) > 5 )?  '<b>Passed</b>' : '<b style="color:red">Failed</b>'  ;
                                    ?>
                                  
                                    <tr>
                                            <td><?php echo $namerst[$ky]; ?> </td>
                                            <td><?php  echo $clas[Input::get('class')]. $studclass[$stdcls[$ky]]; ?> </td>
                                            
                                            <td><?php echo $stud_pos[$ky]  ?> </td>
                                            <td><?php echo $overcls_pos[$stdcls[$ky]][$ky] .' out of '.count($overcls_pos[$stdcls[$ky]]); ?></td>
                                            <td><?php echo $over_pos[$ky] .' out of '.count($over_pos); ?></td>
                                            <td><?php echo (in_array(26, $userpass[$ky])||in_array(27, $userpass[$ky])||in_array(31, $userpass[$ky])||in_array(40, $userpass[$ky])|| count($userpass[$ky]) > 4) ?  '<b style="color:red">Failed</b>' : '<b>Passed</b>'; ?></td>
                                            
                                            <td><?php if(isset($usercom[$ky])) {echo '<button  onclick="showc(\''.$ky .'\')" type="button" class="btn btn-outline btn-info" data-toggle="modal" data-target="#myModal"><i class="fa fa-comments"></i> View</button>'; }else{echo 'No comment yet';} ?></td>
                                            <td>
                                            <?php 
                                            $link = "?SJBSI=7&class=".$stdcls[$ky]."&id=".$ky.'&level='.Input::get('class');
                                            if (!isset($usercom[$ky][$priority])) {
...
<blockquote >
  ...
  <h5 > ${$stud[0]} </h5>
  <footer>Commented By <cite title="Source Title" >${$stud[1]}</cite></footer>
</blockquote> `;
   } 
